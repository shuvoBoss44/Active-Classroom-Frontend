...
export default function CourseCard({ course, progress }) {
  ...
  return (
    <div className="group relative bg-white rounded-2xl sm:rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-emerald-500/10 transition-all duration-500 hover:-translate-y-1 flex flex-col h-full">
      {/* Thumbnail Section */}
      <div className="relative aspect-video overflow-hidden bg-gray-100">
        ...
        <div className="absolute top-3 left-3 sm:top-4 sm:left-4 flex flex-wrap gap-2">
          <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-white/90 backdrop-blur-md text-[10px] sm:text-xs font-bold text-gray-900 shadow-sm flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-emerald-500" />
            {classType}
          </span>
          {hasDiscount && !isOwned && (
            <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-rose-500 text-[10px] sm:text-xs font-bold text-white shadow-sm animate-pulse">
              -{discountPercent}%
            </span>
          )}
          {isOwned && (
             <span className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-emerald-500 text-[10px] sm:text-xs font-bold text-white shadow-sm flex items-center gap-1">
      ...
      <div className="p-4 sm:p-6 flex flex-col flex-grow">
        {/* Meta Info */}
        <div className="flex items-center gap-3 sm:gap-4 text-xs font-medium text-gray-500 mb-3">
