...
import { Loader2, Clock, FileText, CheckCircle, ArrowRight, AlertCircle } from "lucide-react";
import { Button } from "@/components/ui/Button";
import Link from "next/link";

export default function CourseExamsPage({ params }) {
  ...
  const fetchExams = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/exams/course/${courseId}`, {
        credentials: "include",
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-black text-gray-900 mb-8 tracking-tight">Course Exams</h1>
        ...
        {exams.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center border border-gray-100 shadow-sm">
            ...
            <p className="text-gray-500">There are currently no exams scheduled for this course.</p>
          </div>
        ) : (
